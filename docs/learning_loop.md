...
**每个实验内部**：把全部变体的 ΔΔG 同时对"该变体在各框架位点是否携带
human 残基"做 **岭回归**（`learning.py::_ridge_solve`，λ=1），解出每位点
的净效应 β_p。这样 V2 这类**多位点变体**的活性损失会被拆分到真正变化的
位点，而不是平均摊派到每个差异位点。

- 当变体数 < 差异位点数（如只测了一个 V0，欠定）时，回退到
  "该变体 ΔΔG 平均归因到其差异位点"的启发式，保证流程可继续。
- **收缩**：按样本量 n 向 0 收缩（`effect·n/(n+1)`）。
- 跨实验合并：同一位点按样本量加权平均。
- **替换对匹配**：`backmut.py` 只在 `donor_aa`/`human_aa` 与候选完全一致时
  应用该位点的实测效应（同一位点换一个 germline 就是另一种替换）。
